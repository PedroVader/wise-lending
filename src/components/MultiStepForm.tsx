...
const isValid = (key: string, value: string) => {
  ...
  if (key === "email") return /^[\w.-]+@[\w.-]+\.[a-zA-Z]{2,}$/.test(value);
  if (key === "phone") return value.replace(/\D/g, "").length === 10;
  if (key === "zipCode") return /^\d{5}(-\d{4})?$/.test(value);
  return true;
};
...
const getAirtableConfig = () => {
  ...
  if (!baseId || !tableName || !token) throw new Error("Missing Airtable env vars");
  return {
...
const sendToAirtable = async (data: any) => {
  ...
  Object.keys(data).forEach((key) => {
    if (data[key] && fieldMapping[key]) fields[fieldMapping[key]] = data[key];
  });
...
const updateAirtableRecord = async (recordId: string, data: any, isComplete = false) => {
  const config = getAirtableConfig();
  const fields: Record<string, string> = {};
  Object.keys(data).forEach((key) => {
    if (data[key] && fieldMapping[key]) fields[fieldMapping[key]] = data[key];
...
const MultiStepForm = () => {
  ...
  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      ...
      if (airtableRecordId) updateAirtableRecord(airtableRecordId, finalData, true);
                ...
                {step.options.map(option => (
                  <button
                    ...
                    className={`w-full px-4 py-3 border rounded-lg text-center font-medium transition-colors ${
                      value === option ? "border-green-500 bg-green-100" : "border-gray-300 bg-white hover:bg-gray-50"
                    }`}
                  >
                    {option}
                  </button>
